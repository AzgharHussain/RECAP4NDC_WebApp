...
const CoupeObservation = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://68.178.167.39:5000/api/coupe/log-with-images?user_id=2');
  ...
  const applyFilters = () => {
    ...
    if (searchOfficer.trim() !== "") {
      ...
      data = data.filter(item => item.p_officer_name?.toLowerCase().includes(lowerSearch));
    }
    ...
    if (selectedDate) {
      data = data.filter(item => dayjs(item.p_date_time).isSame(selectedDate, 'day'));
    }
 ...
 const columns = [
  ...
  },
  {
    ...
    sorter: (a, b) =>
      (a.p_observation_notes || '').localeCompare(b.p_observation_notes || ''),
  },
  {
    title: 'Images',
  ...
  const handleExport = () => {
    ...
    saveAs(new Blob([wbout], { type: "application/octet-stream" }), 'Coupe_Observation_Logs.xlsx');
  };
